...
export default function ViewerPage() {
  ...
  const saveCell = async (date: string, time: string) => {
    await fetch("/api/saveMeal", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ date, time, food: cellValue })
    })
    setEditingCell(null)
    loadGrid()
  }
  ...
  return (
    <div style={{ padding: "30px", fontFamily: "Arial, sans-serif", maxWidth: "1200px", margin: "auto" }}>
      <h1 style={{ textAlign: "center", marginBottom: "30px" }}>Weekly Diet Tracker</h1>
      ...
      <div style={{ overflowX: "auto", boxShadow: "0 2px 8px rgba(0,0,0,0.1)", borderRadius: "8px" }}>
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: "700px" }}>
          <thead style={{ backgroundColor: "#1976d2", color: "#fff" }}>
            <tr>
              <th style={{ padding: "10px", position: "sticky", left: 0, backgroundColor: "#1976d2" }}>Time</th>
              {weekDates.map(date => {
                ...
                return <th key={date} style={{ padding: "10px" }}>{dayName}<br />{formattedDate}</th>
              })}
            </tr>
          </thead>
          <tbody>
            {times.map(time => (
              <tr key={time} style={{ backgroundColor: "#fafafa" }}>
                <td style={{ padding: "8px", fontWeight: "bold", position: "sticky", left: 0, backgroundColor: "#f0f0f0", borderRight: "1px solid #ccc" }}>{time}</td>
                {weekDates.map(date => {
                  ...
                  return (
                    <td
                      ...
                      onMouseEnter={e => {
                        if (editingCell !== cellId) e.currentTarget.style.backgroundColor = "#f1f7ff"
                      }}
                      onMouseLeave={e => {
                        if (editingCell !== cellId) e.currentTarget.style.backgroundColor = editingCell === cellId ? "#fff9c4" : "#fff"
                      }}
                    >
                      {editingCell === cellId ? (
                        <input
                          ...
                          onKeyDown={async e => {
                            if (e.key === "Enter") saveCell(date, time)
                          }}
